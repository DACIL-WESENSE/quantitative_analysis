...
This recurses through `data/` and produces a `.csv` alongside each `.xls`.

## Architecture

```
main.ipynb          # orchestrates the full pipeline: load → EDA → ML → report
functions.py        # all reusable helpers (imported as `fn`)
xls_to_csv.py       # one-off data preparation utility
...
6. Export (`save_telemetry_csv`, `build_patient_summary`)
7. Utilities (`setup_logging`, `_save_figure`, `_find_column`)
